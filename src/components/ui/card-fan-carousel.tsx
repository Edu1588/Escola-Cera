...
function getSlotConfig(totalCards: number, slot: number) {
  ...
  // Map smaller arrays to the center explicit slots
  const centerIdx = HALF;
  const mappedSlot = centerIdx - (totalCards >> 1) + slot;
  return FAN_POSITIONS[mappedSlot];
}

const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-black/10 bg-black/5 text-black/40 cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.1)] hover:border-black/25 hover:text-black/70 active:opacity-70 transition-colors duration-300 before:content-[''] before:absolute before:inset-[3px] before:rounded-full before:border before:border-black/[0.04] before:pointer-events-none";
